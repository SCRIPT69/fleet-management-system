...
public interface ClientCompanyRepository extends JpaRepository<ClientCompany, String> {
    ...
    @Query("SELECT c FROM ClientCompany c WHERE c NOT IN " +
            "(SELECT DISTINCT o.clientCompany FROM Order o WHERE o.orderDate >= :since)")
    List<ClientCompany> findInactiveClients(@Param("since") LocalDate since);
}